...
# Helpers locais (iguais aos de Entradas)
...
def render(df_saidas: pd.DataFrame) -> None:
    """
    Renderiza a visão de 📤 Saídas usando um DataFrame padronizado com colunas:
      - 'Data' (datetime64)
      - 'Valor' (float)
    ...
    # 3) Layout em 2 colunas (igual Entradas)
    col_esq, col_dir = st.columns(2)

    # 3.1) ESQUERDA — Total por mês (sem índice e sem scroll)
    with col_esq:
    ...
    with col_dir:
        ...
        if mes is None or df_mes.empty:
            ...
            st.dataframe(
                ...
                use_container_width=True,
                hide_index=True,
                height=altura_esq,  # mesma altura → scroll apenas aqui se exceder
            )
